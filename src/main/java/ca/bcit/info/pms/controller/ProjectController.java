...
public class ProjectController implements Serializable {
	...
	public String addProject() {
		...
		if (backProject != null) {
			FacesContext.getCurrentInstance().addMessage(
					"newProjectForm:itProjectNumber",
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "",
							"Project already exist!: " + project.getId()));
			return null;
		}
	...
	public String updateAssistant() {
		...
		final Employee assistant = empService
				.findEmployeeByUsername(assistantUsername);
		if (assistant == null) {
			FacesContext.getCurrentInstance().addMessage(
					"projectForm:mnAssistant",
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "",
							"No employee found with username: "
									+ assistantUsername));
	...
	public List<Project> getManagedProjects() {
		...
		final boolean isManager = userController
				.isAuthorized(Employee.ROLE_PROJECT_MANAGER);
		final boolean isAssistant = userController
				.isAuthorized(Employee.ROLE_ASSISTANT);
		if (isManager && isAssistant) {
			final List<Project> assistantList = projService
					.getAssistedProjectsFor(userId);
	...
	public List<Employee> getProjectManagerCandidates() {
		final List<Employee> candidates = employeeController
				.getManagedEmployees();
		...
		candidates.sort((o1, o2) -> o1.getLastName()
